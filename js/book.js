...
function capColor(status) {
  if (status === 'full') return '#991B1B';
  if (status === 'nearly') return '#B45309';
  return 'var(--ink)';
}
...
document.getElementById('book-class-card').innerHTML = `
  <div class="bcc-header">
    <div class="bcc-emoji">${cls.emoji}</div>
    <div>
  ...
  <div class="bcc-details">
    <div class="bcc-detail-item">
      <span class="bcc-detail-label">Instructor</span>
      <span class="bcc-detail-val">${cls.instructor}</span>
    </div>
    <div class="bcc-detail-item">
      <span class="bcc-detail-label">Age group</span>
      <span class="bcc-detail-val">${cls.age}</span>
    </div>
    <div class="bcc-detail-item">
      <span class="bcc-detail-label">Duration</span>
      <span class="bcc-detail-val">30 minutes</span>
    </div>
    <div class="bcc-detail-item">
      <span class="bcc-detail-label">Price</span>
      <span class="bcc-detail-val">1 lesson from balance</span>
    </div>
  </div>
  <div class="cap-bar-wrap" style="margin-top:16px">
    <div class="cap-bar">
      <div class="cap-fill" style="width:${pct}%;background:${clr}"></div>
    </div>
    <div class="cap-label" style="color:${clr}">
      ${full ? 'Class full' : `${cls.taken} of ${cls.total} spots taken`}
    </div>
  </div>`;
...
if (nap) {
  ...
  txt.textContent =
    'This class overlaps with your child\'s nap time. ' +
  ...
  list.innerHTML = APP.children.map(child => `
    <div class="book-child ${selectedChildId === child.id ? 'selected' : ''}"
      ...
      <div class="book-child-left">
        ...
        <div>
          <div class="book-child-name">${child.name}</div>
          <div class="book-child-age">${child.age}</div>
        </div>
      </div>
      <div class="book-child-right">
        <span class="consent-chip ${child.consentStatus === 'ok' ? 'ok' : 'pending'}">
          ${child.consentStatus === 'ok' ? 'Consent ✓' : 'Consent pending'}
        </span>
      </div>
    </div>`).join('');
...
function selectChild(id) {
  ...
  const child = APP.children.find(c => c.id === id);
  updateBookingState(child);
}
...
function updateBookingState(child) {
  ...
  if (child.consentStatus !== 'ok') {
    ...
    confirmBtn.textContent = 'Consent required';
  ...
  if (APP.lessonBalance < 1) {
    ...
    confirmBtn.disabled = true;
    confirmBtn.textContent = 'No lessons in balance';
    confirmBtn.style.opacity = '0.5';
    return;
  }
